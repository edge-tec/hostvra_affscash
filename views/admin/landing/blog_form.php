<?php require BASE_PATH . '/views/layouts/admin.php'; ?>

<!-- Quill rich text editor -->
<link href="https://cdn.quilljs.com/1.3.7/quill.snow.css" rel="stylesheet">
<script src="https://cdn.quilljs.com/1.3.7/quill.min.js"></script>

<div class="page-header">
    <div>
        <h1><?= $postId ? '✏ Edit Post' : '+ Create Blog Post' ?></h1>
        <p><?= $postId ? 'Update this landing page blog post' : 'Write a new blog post for the landing page' ?></p>
    </div>
    <a href="/admin/landing/blog" class="btn btn-secondary">← Back to Blog</a>
</div>

<?php if (!empty($errors)): ?>
<div class="alert alert-danger mb-3">
    <?php foreach ($errors as $e): ?><div>⚠ <?= Helpers::e($e) ?></div><?php endforeach; ?>
</div>
<?php endif; ?>

<style>
.admin-form-grid {
    display: grid;
    grid-template-columns: 1fr 320px;
    gap: 20px;
    align-items: start;
}
@media (max-width: 991px) {
    .admin-form-grid {
        grid-template-columns: 1fr;
    }
}
</style>

<form method="POST" enctype="multipart/form-data" id="blog-form">
    <?= Helpers::csrf() ?>

    <div class="admin-form-grid">

        <!-- Main content -->
        <div>
            <div class="card mb-3">
                <div class="card-header"><span class="card-title">Article Content</span></div>
                <div class="card-body">
                    <div class="form-group">
                        <label style="font-weight:700">Title <span style="color:#EF4444">*</span></label>
                        <input type="text" name="title" class="form-control"
                               value="<?= Helpers::e($post['title'] ?? '') ?>"
                               placeholder="Enter article title…" required
                               style="font-size:18px;font-weight:600;height:48px">
                    </div>
                    <div class="form-group">
                        <label style="font-weight:700">Excerpt</label>
                        <textarea name="excerpt" class="form-control" rows="2"
                                  placeholder="Brief description shown in blog cards…"
                                  style="resize:vertical"><?= Helpers::e($post['excerpt'] ?? '') ?></textarea>
                        <div style="font-size:11px;color:#94A3B8;margin-top:3px">Shown in the blog card preview on the landing page.</div>
                    </div>
                    <div class="form-group">
                        <label style="font-weight:700">Full Content</label>
                        <div id="quill-editor" style="min-height:280px;background:#fff;border:1px solid #E2E8F0;border-radius:0 0 8px 8px"></div>
                        <input type="hidden" name="body" id="body-input" value="<?= Helpers::e($post['body'] ?? '') ?>">
                    </div>
                </div>
            </div>

            <!-- SEO Keywords Module Section -->
            <?php require BASE_PATH . '/views/admin/landing/seo_keywords_section.php'; ?>
        </div>

        <!-- Sidebar settings -->
        <div>
            <!-- Publish box -->
            <div class="card mb-3">
                <div class="card-header"><span class="card-title">Publish Settings</span></div>
                <div class="card-body">
                    <div class="form-group">
                        <label style="font-weight:700">Status</label>
                        <select name="status" class="form-control" id="blog-status-select" onchange="syncBtn()">
                            <option value="published" <?= ($post['status'] ?? 'published') === 'published' ? 'selected' : '' ?>>● Published</option>
                            <option value="draft"     <?= ($post['status'] ?? '') === 'draft' ? 'selected' : '' ?>>○ Draft</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label style="font-weight:700">Category</label>
                        <input type="text" name="category" class="form-control"
                               value="<?= Helpers::e($post['category'] ?? '') ?>"
                               placeholder="e.g. Affiliate Tips, News" list="category-list">
                        <?php if (!empty($categories)): ?>
                        <datalist id="category-list">
                            <?php foreach ($categories as $c): ?>
                            <option value="<?= Helpers::e($c['category']) ?>">
                            <?php endforeach; ?>
                        </datalist>
                        <?php endif; ?>
                    </div>

                    <!-- Featured flag -->
                    <div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;padding:12px 14px;margin-bottom:16px">
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-weight:700;color:#92400E">
                            <input type="checkbox" name="is_featured" value="1"
                                   <?= ($post['is_featured'] ?? 0) ? 'checked' : '' ?>
                                   style="width:18px;height:18px;accent-color:#F59E0B">
                            <div>
                                <div style="font-size:14px">⭐ Featured Post</div>
                                <div style="font-size:11px;font-weight:400;color:#B45309;margin-top:2px">Highlighted in the blog section</div>
                            </div>
                        </label>
                    </div>

                    <button type="submit" class="btn btn-primary" id="blog-submit-btn" style="width:100%;justify-content:center">
                        <?= $postId ? '💾 Save Changes' : '🚀 Publish Post' ?>
                    </button>
                    <?php if ($postId): ?>
                    <a href="/admin/landing/blog" class="btn btn-secondary" style="width:100%;justify-content:center;margin-top:8px">Cancel</a>
                    <?php endif; ?>
                </div>
            </div>

            <!-- Featured image -->
            <div class="card">
                <div class="card-header"><span class="card-title">Featured Image</span></div>
                <div class="card-body">
                    <?php if (!empty($post['image'])): ?>
                    <div style="margin-bottom:12px;border-radius:8px;overflow:hidden">
                        <img src="<?= Helpers::e($post['image']) ?>" alt="" style="width:100%;max-height:240px;object-fit:contain;background:#0f0826;border-radius:8px;display:block">
                    </div>
                    <?php endif; ?>
                    <input type="file" name="image" class="form-control"
                           accept="image/jpeg,image/png,image/gif,image/webp" id="img-input">
                    <div id="img-preview" style="display:none;margin-top:10px;border-radius:8px;overflow:hidden">
                        <img id="img-preview-img" src="" alt="" style="width:100%;max-height:240px;object-fit:contain;background:#0f0826;border-radius:8px;display:block">
                    </div>
                    <div style="font-size:11px;color:#94A3B8;margin-top:6px">Recommended: 1200×630px JPEG/PNG</div>
                </div>
            </div>
        </div>
    </div>
</form>

<script>
function syncBtn() {
    var sel = document.getElementById('blog-status-select');
    var btn = document.getElementById('blog-submit-btn');
    if (!sel || !btn || <?= $postId ? 'true' : 'false' ?>) return;
    var isDraft = sel.value === 'draft';
    btn.textContent = isDraft ? '💾 Save as Draft' : '🚀 Publish Post';
    btn.style.background   = isDraft ? '#64748B' : '';
    btn.style.borderColor  = isDraft ? '#64748B' : '';
}
document.addEventListener('DOMContentLoaded', syncBtn);

var quill = new Quill('#quill-editor', {
    theme: 'snow',
    placeholder: 'Write your blog content here…',
    modules: {
        toolbar: [
            [{ 'header': [1, 2, 3, false] }],
            ['bold', 'italic', 'underline', 'strike'],
            [{ 'color': [] }, { 'background': [] }],
            [{ 'list': 'ordered' }, { 'list': 'bullet' }],
            ['link', 'image'],
            ['blockquote', 'code-block'],
            ['clean']
        ]
    }
});
var existingBody = document.getElementById('body-input').value;
if (existingBody) quill.root.innerHTML = existingBody;
document.getElementById('blog-form').addEventListener('submit', function() {
    document.getElementById('body-input').value = quill.root.innerHTML;
});

document.getElementById('img-input').addEventListener('change', function() {
    var file = this.files[0];
    if (!file) return;
    var reader = new FileReader();
    reader.onload = function(e) {
        document.getElementById('img-preview-img').src = e.target.result;
        document.getElementById('img-preview').style.display = 'block';
    };
    reader.readAsDataURL(file);
});
</script>

<?php require BASE_PATH . '/views/layouts/admin_footer.php'; ?>
