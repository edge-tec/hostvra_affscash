...
<div class="page-header">
    <div>
        ...
        <p>Review and manage affiliate requests for smartlink access</p>
    </div>
    <div class="d-flex gap-2">
        <a href="/admin/smartlinks" class="btn btn-secondary">← Smartlinks</a>
    </div>
</div>
...
<div style="display:flex;gap:4px;margin-bottom:20px;border-bottom:2px solid #E2E8F0;padding-bottom:0">
    <?php foreach(['pending'=>'Pending','approved'=>'Approved','rejected'=>'Rejected','all'=>'All'] as $val=>$label):
        $count = '';
        if ($val === 'pending') $count = ' (' . (int)($pendingCount['cnt'] ?? 0) . ')';
    ?>
    <a href="/admin/smartlinks?action=requests&status=<?= $val ?>"
       style="padding:10px 18px;font-size:13px;font-weight:600;border-radius:8px 8px 0 0;text-decoration:none;color:<?= $statusFilter===$val?'#4F46E5':'#64748B' ?>;border-bottom:<?= $statusFilter===$val?'2px solid #4F46E5':'2px solid transparent' ?>;margin-bottom:-2px">
        <?= $label . $count ?>
    </a>
    <?php endforeach; ?>
</div>
...
<div class="card">
    <div class="card-body text-center text-muted" style="padding:48px">
        ...
        <div style="font-size:13px">Affiliate smartlink access requests will appear here.</div>
    </div>
</div>
<?php else: ?>
<div class="card">
        ...
        <table>
            ...
            <tr>
                <td>
                    ...
                    <div class="text-sm text-muted"><?= Helpers::e($r['aff_email']) ?></div>
                </td>
                <td>
                ...
                <td class="text-sm"><?= $r['admin_note'] ? Helpers::e($r['admin_note']) : '<span class="text-muted">—</span>' ?></td>
                <td class="text-sm text-muted">
                    <?= $r['reviewed_at'] ? date('M j, Y', strtotime($r['reviewed_at'])) : '—' ?>
                </td>
                <td>
...
<div id="review-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center">
    <div style="background:#fff;border-radius:12px;padding:32px;max-width:480px;width:90%">
        <h3 id="modal-title" style="margin-bottom:8px"></h3>
        <p id="modal-desc" style="color:#64748B;font-size:13px;margin-bottom:20px"></p>
        <form method="POST" action="/admin/smartlinks?action=review_request">
            <?= Helpers::csrf() ?>
            <input type="hidden" name="request_id" id="modal-req-id">
            <input type="hidden" name="decision" id="modal-decision">
            <div class="form-group">
                <label>Admin Note <span class="text-muted" style="font-weight:400">(optional — shown to affiliate)</span></label>
                <textarea name="admin_note" class="form-control" rows="3" placeholder="Reason for approval or rejection…"></textarea>
            </div>
...
function openReview(reqId, affName, slName, decision) {
    ...
    document.getElementById('modal-desc').textContent  = affName + ' requesting access to "' + slName + '"';
    var btn = document.getElementById('modal-submit-btn');
    btn.className = 'btn ' + (decision === 'approved' ? 'btn-success' : 'btn-danger');
    btn.textContent = decision === 'approved' ? 'Approve Access' : 'Reject Request';
