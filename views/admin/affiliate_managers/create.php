<?php
$pageTitle = 'Create Affiliate Manager';
require BASE_PATH . '/views/layouts/admin.php';
?>

<div class="page-header">
    <div><h1>Create Affiliate Manager</h1><p>Create a new manager account and set permissions</p></div>
...
<form method="POST" action="/admin/affiliate-managers/create">
    <?= Helpers::csrf() ?>
    <div class="grid-2">
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-row cols-2">
                    <div class="form-group"><label>First Name *</label><input type="text" name="first_name" class="form-control" required value="<?= Helpers::e($_POST['first_name']??'') ?>"></div>
                    <div class="form-group"><label>Last Name *</label><input type="text" name="last_name" class="form-control" required value="<?= Helpers::e($_POST['last_name']??'') ?>"></div>
                </div>
                <div class="form-group"><label>Email *</label><input type="email" name="email" class="form-control" required value="<?= Helpers::e($_POST['email']??'') ?>"></div>
                ...
                <div class="form-group"><label>Notes</label><textarea name="notes" class="form-control" rows="3"><?= Helpers::e($_POST['notes']??'') ?></textarea></div>
                <div class="form-group" style="background:#FAFAF9;border:1px solid #E5E7EB;border-radius:8px;padding:14px">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                        <input type="number" name="commission_rate" class="form-control" min="0" max="100" step="0.01"
                               value="<?= Helpers::e($_POST['commission_rate'] ?? '0') ?>" style="max-width:140px" placeholder="0.00">
                        <span style="font-size:12px;color:#6B7280">% of (Advertiser Revenue &minus; Affiliate Payout). This figure is <strong>never shown</strong> to the manager.</span>
                    </div>
                </div>
                <hr style="margin:16px 0">
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
                    <div class="form-group"><label style="display:flex;align-items:center;gap:5px"><span style="color:#00AFF0">&#128222;</span> Skype</label><input type="text" name="skype" class="form-control" placeholder="live:username" value="<?= Helpers::e($_POST['skype']??'') ?>"></div>
                    <div class="form-group"><label style="display:flex;align-items:center;gap:5px"><span style="color:#2AABEE">&#128232;</span> Telegram</label><input type="text" name="telegram" class="form-control" placeholder="@username" value="<?= Helpers::e($_POST['telegram']??'') ?>"></div>
                    <div class="form-group"><label style="display:flex;align-items:center;gap:5px"><span style="color:#5865F2">&#127918;</span> Discord</label><input type="text" name="discord" class="form-control" placeholder="username" value="<?= Helpers::e($_POST['discord']??'') ?>"></div>
                </div>
            </div>
        </div>
