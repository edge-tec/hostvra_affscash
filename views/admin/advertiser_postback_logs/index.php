...
<div class="page-header">
    <div>
        <h1>Advertiser Postback Log</h1>
        <p>Incoming postbacks from advertisers — all statuses, all advertisers</p>
    </div>
...
<div class="stat-grid-6">
    ...
    <div class="card" style="margin:0"><div class="card-body" style="padding:14px 18px">
        <div class="text-muted" style="font-size:11px;font-weight:700;text-transform:uppercase">Accepted</div>
        <div style="font-size:24px;font-weight:700;color:#059669;margin-top:4px"><?= number_format($stats['accepted'] ?? 0) ?></div>
    </div></div>
    <div class="card" style="margin:0"><div class="card-body" style="padding:14px 18px">
        <div class="text-muted" style="font-size:11px;font-weight:700;text-transform:uppercase">Duplicate</div>
        <div style="font-size:24px;font-weight:700;color:#D97706;margin-top:4px"><?= number_format($stats['duplicate'] ?? 0) ?></div>
    </div></div>
    <div class="card" style="margin:0"><div class="card-body" style="padding:14px 18px">
        <div class="text-muted" style="font-size:11px;font-weight:700;text-transform:uppercase">Rejected</div>
        <div style="font-size:24px;font-weight:700;color:#DC2626;margin-top:4px"><?= number_format($stats['rejected'] ?? 0) ?></div>
    </div></div>
    <div class="card" style="margin:0"><div class="card-body" style="padding:14px 18px">
        <div class="text-muted" style="font-size:11px;font-weight:700;text-transform:uppercase">Accept Rate</div>
        <div style="font-size:24px;font-weight:700;color:<?= $acceptRate>=90?'#059669':($acceptRate>=70?'#D97706':'#DC2626') ?>;margin-top:4px"><?= $acceptRate ?>%</div>
    </div></div>
    <div class="card" style="margin:0"><div class="card-body" style="padding:14px 18px">
        <div class="text-muted" style="font-size:11px;font-weight:700;text-transform:uppercase">Payout</div>
        <div style="font-size:24px;font-weight:700;margin-top:4px">$<?= number_format($stats['total_payout'] ?? 0, 2) ?></div>
    </div></div>
</div>

<!-- Filters -->
<div class="card mb-3 filter-card filter-open" id="apl-filter-card">
    ...
    <div class="card-body" style="padding:14px 20px">
        <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
            ...
            <div class="form-group mb-0" style="min-width:180px">
                <label style="font-size:12px">Advertiser</label>
                <select name="advertiser_id" class="form-control" style="font-size:13px">
                    <option value="">All Advertisers</option>
                    <?php foreach ($advertiserList as $a): ?>
                    <option value="<?= $a['id'] ?>" <?= $filterAdv===$a['id']?'selected':'' ?>><?= Helpers::e($a['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div style="display:flex;gap:8px;padding-bottom:1px">
                ...
                <a href="/admin/advertiser-postback-logs" class="btn btn-secondary btn-sm">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card">
    <div class="card-header" style="display:flex;align-items:center;justify-content:space-between">
        <span class="card-title">Log Entries (<?= number_format(count($logs)) ?> shown)</span>
    ...
    <div class="table-wrap" style="overflow-x:auto">
        <table style="font-size:12px">
            ...
            <tbody>
            <?php if (empty($logs)): ?>
            <tr><td colspan="10" class="text-center text-muted" style="padding:40px">No records found for this period</td></tr>
            <?php else: ?>
            ...
            <tr style="<?= $rowBg ?>">
                ...
                <td style="white-space:nowrap;color:#64748B;font-size:11px"><?= Helpers::e(date('M j, Y H:i:s', strtotime($log['created_at']))) ?></td>
                <td>
                    <?php if (!empty($log['adv_name'])): ?>
                    <span class="fw-bold text-sm"><?= Helpers::e($log['adv_name']) ?></span>
                    <?php else: ?>
                    <span class="text-muted text-sm">ID:<?= $log['advertiser_id'] ?? '—' ?></span>
                    <?php endif; ?>
                </td>
                ...
                <td><?= $log['payout'] > 0 ? '<strong>$'.number_format((float)$log['payout'],4).'</strong>' : '<span class="text-muted">—</span>' ?></td>
                <td>
                ...
                <td style="color:#64748B;font-size:11px;max-width:200px"><?= Helpers::e($log['reject_reason'] ?? '') ?></td>
                <td style="font-family:monospace;font-size:11px;color:#94A3B8"><?= Helpers::e($log['request_ip'] ?? '') ?></td>
            </tr>
            <?php endforeach; ?>
            <?php endif; ?>
            </tbody>
