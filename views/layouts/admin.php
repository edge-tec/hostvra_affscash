...
<div class="sidebar-group">
    ...
    <a href="/admin/reports/traffic-source-override" class="nav-link <?= str_starts_with($_SERVER['REQUEST_URI'],'/admin/reports/traffic-source-override') ? 'active' : '' ?>">
        ...
        Traffic Source Override Logs
    </a>
    <a href="/admin/traffic" class="nav-link <?= str_contains($_SERVER['REQUEST_URI'],'/admin/traffic') && !str_contains($_SERVER['REQUEST_URI'],'/admin/traffic-source-override') ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        Traffic Stats
    </a>
    <a href="/admin/traffic-source-override" class="nav-link <?= str_contains($_SERVER['REQUEST_URI'],'/admin/traffic-source-override') ? 'active' : '' ?>">
...
<div class="sidebar-group">
    ...
    <div class="sidebar-group-items">
        <a href="/admin/auto-invoices" class="nav-link <?= str_contains($_SERVER['REQUEST_URI'],'/admin/auto-invoices') ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
        Automatic Invoices
    </a>
    <a href="/admin/invoices" class="nav-link <?= str_contains($_SERVER['REQUEST_URI'],'/admin/invoices') && !str_contains($_SERVER['REQUEST_URI'],'/admin/auto-invoices') ? 'active' : '' ?>">
...
<?php
    ...
    <div class="fds-nav-section">
        ...
        <div class="fds-submenu <?= $_fraudOpen ? 'open' : '' ?>" id="fraudSubmenu">
            ...
            <a href="/admin/fraud-center/report-logs"
               class="fds-link <?= str_starts_with($_SERVER['REQUEST_URI'],'/admin/fraud-center/report-logs') ? 'active' : '' ?>">
...
<header class="topbar" style="position: relative;">
    ...
    <div class="topbar-actions">
        ...
        <!-- Chat bell -->
        <a href="/admin/support" style="position:relative;display:flex;align-items:center;padding:6px;color:var(--text);opacity:0.95;text-decoration:none;border-radius:6px;transition:.2s" title="Live Support" onmouseover="this.style.background='rgba(255,255,255,0.4)'" onmouseout="this.style.background='transparent'">
            ...
            <span id="topbar-chat-badge" style="display:none;position:absolute;top:1px;right:1px;background:#EF4444;color:#fff;min-width:15px;height:15px;border-radius:10px;padding:0 3px;box-sizing:border-box;font-size:9px;font-weight:700;align-items:center;justify-content:center;line-height:1">0</span>
        </a>
        <a href="/admin/notifications" class="notification-btn" style="text-decoration:none; color:var(--text); border:1px solid rgba(255,255,255,0.3); background:rgba(255,255,255,0.25); border-radius:12px; padding:6px; display:flex; align-items:center; justify-content:center; transition:.2s" onmouseover="this.style.background='rgba(255,255,255,0.45)'" onmouseout="this.style.background='rgba(255,255,255,0.25)'">
        ...
        <div class="user-menu" style="position:relative; background:rgba(255,255,255,0.25); border:1px solid rgba(255,255,255,0.3); border-radius:24px; padding:4px 10px 4px 4px;">
            <div class="user-avatar" style="width:24px; height:24px; font-size:11px; background:var(--primary);"><?= strtoupper(substr(Auth::currentUser()['first_name'] ?? 'A', 0, 1)) ?></div>
            <span style="color:var(--text); font-weight: 600; font-size: 13px;"><?= Helpers::e(Auth::currentUser()['first_name'] ?? 'Admin') ?></span>
