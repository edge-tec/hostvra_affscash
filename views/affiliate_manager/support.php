...
<style>
/* ── Conversation List ────────────────────────────────────────────── */
.mc-conv-row{padding:12px 16px;cursor:pointer;border-bottom:1px solid #F1F5F9;transition:background .15s;border-left:3px solid transparent}
.mc-conv-row.active{background:#EEF2FF;border-left-color:#0EA5E9}
.mc-conv-row:hover:not(.active){background:#F8FAFC}

/* ── Date Separator ───────────────────────────────────────────────── */
.mc-date-sep{display:flex;align-items:center;gap:12px;margin:16px 0 8px;user-select:none}
.mc-date-sep::before,.mc-date-sep::after{content:'';flex:1;height:1px;background:#E2E8F0}
.mc-date-sep span{font-size:11px;font-weight:600;color:#94A3B8;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap;padding:4px 12px;background:#F1F5F9;border-radius:12px;border:1px solid #E2E8F0}

/* ── Message Row ──────────────────────────────────────────────────── */
.mc-msg-row{display:flex;flex-direction:column;position:relative;max-width:100%;padding:2px 0}
.mc-msg-row.mine{align-items:flex-end}
.mc-msg-row.theirs{align-items:flex-start}

/* ── Bubble ───────────────────────────────────────────────────────── */
.mc-bubble{max-width:min(72%,420px);padding:10px 14px;border-radius:18px 18px 18px 6px;background:#fff;color:#1E293B;font-size:13.5px;line-height:1.55;box-shadow:0 1px 3px rgba(0,0,0,.06);border:1px solid #E2E8F0;word-wrap:break-word;overflow-wrap:break-word;overflow:hidden}
.mc-msg-row.mine .mc-bubble{background:linear-gradient(135deg,#0EA5E9,#3B82F6);color:#fff;border:none;border-radius:18px 18px 6px 18px;box-shadow:0 2px 8px rgba(14,165,233,.25)}

/* ── Meta ─────────────────────────────────────────────────────────── */
.mc-meta{font-size:10px;color:#94A3B8;margin-bottom:2px;padding:0 4px}
.mc-msg-row.mine .mc-meta{text-align:right}
.mc-edited{font-size:10px;color:#94A3B8;font-style:italic;margin-left:5px}
.mc-msg-row.mine .mc-edited{color:rgba(255,255,255,.6)}

/* ── Actions ──────────────────────────────────────────────────────── */
.mc-actions{display:none;gap:4px;background:#fff;border:1px solid #E2E8F0;border-radius:16px;padding:2px 6px;box-shadow:0 2px 6px rgba(0,0,0,.06);margin-top:2px}
.mc-msg-row:hover .mc-actions{display:inline-flex}
.mc-action-btn{background:none;border:none;font-size:13px;color:#64748B;cursor:pointer;padding:2px 5px;border-radius:50%;transition:all .15s}
.mc-action-btn:hover{background:#F1F5F9;color:#0F172A}
.mc-action-btn.danger:hover{background:#FEE2E2;color:#B91C1C}

/* ── Attachments ──────────────────────────────────────────────────── */
.mc-att-img{max-width:100%;width:auto;max-height:280px;border-radius:10px;display:block;margin-top:6px;cursor:pointer;object-fit:contain}
.mc-att-file{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;background:#F1F5F9;border:1px solid #E2E8F0;border-radius:10px;font-size:12px;color:#1E293B;text-decoration:none;margin-top:6px;transition:background .15s;max-width:100%;overflow:hidden}
.mc-att-file:hover{background:#EEF2FF}
.mc-att-icon{width:30px;height:30px;background:#EEF2FF;color:#0EA5E9;border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:10px;flex-shrink:0;text-transform:uppercase}
.mc-att-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:160px}

/* ── Status Badge ─────────────────────────────────────────────────── */
.mc-status-badge{font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;text-transform:uppercase;letter-spacing:.04em;display:inline-flex;align-items:center;gap:4px}
.mc-status-badge.open{background:#D1FAE5;color:#065F46}
.mc-status-badge.closed{background:#FEE2E2;color:#991B1B}

/* ── Messages Area ────────────────────────────────────────────────── */
.mc-messages{flex:1;overflow-y:auto;overflow-x:hidden;padding:16px 20px;display:flex;flex-direction:column;gap:4px;background:linear-gradient(180deg,#F8FAFC 0%,#F1F5F9 100%)}
...
.mc-translate-group select{background:none;border:none;outline:none;font-size:11px;padding:0 6px;color:#475569;font-weight:600;cursor:pointer}
.mc-translate-group button{background:none;border:none;border-left:1px solid #E2E8F0;height:100%;padding:0 10px;font-size:12px;color:#0F172A;cursor:pointer;font-weight:600;white-space:nowrap}
.mc-translate-group button:hover{background:#E2E8F0}
.mc-att-preview{font-size:12px;color:#475569;background:#E0F2FE;border:1px solid #BAE6FD;border-radius:10px;padding:8px 12px;margin-bottom:8px;display:flex;align-items:center;gap:8px}

/* ── Mobile Back Button ───────────────────────────────────────────── */
.mc-back-btn{display:none;background:none;border:none;cursor:pointer;font-size:13px;color:#0EA5E9;font-weight:600;padding:4px 8px;border-radius:6px}
.mc-back-btn:hover{background:#E0F2FE}

/* ── Responsive ───────────────────────────────────────────────────── */
@media(max-width:780px){
    #mc-support-grid{grid-template-columns:1fr !important;height:auto !important}
    #mc-conv-list-card{height:50vh !important}
    #mc-chat-panel{height:70vh !important}
    #mc-chat-panel.mc-panel-active{display:flex !important}
    .mc-back-btn{display:inline-flex !important}
    .mc-bubble{max-width:min(85%,320px)}
    .mc-input-row{flex-wrap:wrap}
    .mc-translate-group{order:3;width:100%}
}
@media(max-width:480px){
    .mc-bubble{max-width:90%;padding:8px 10px;font-size:13px}
    .mc-send-btn span{display:none}
    .mc-send-btn{width:44px;padding:0;justify-content:center}
    .mc-att-img{max-height:200px}
}
</style>

<div id="mc-support-grid" style="display:grid;grid-template-columns:300px 1fr;gap:16px;height:calc(100vh - 200px);min-height:500px">

    <!-- Left: Conversation List -->
    <div class="card" id="mc-conv-list-card" style="display:flex;flex-direction:column;overflow:hidden;padding:0">
    ...
    <!-- Right: Chat Panel -->
    <div class="card" id="mc-chat-panel" style="display:flex;flex-direction:column;overflow:hidden;padding:0">
        <div id="chat-empty" style="flex:1;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:14px;color:var(--text-muted)">
            <div style="width:72px;height:72px;background:#E0F2FE;border-radius:50%;display:flex;align-items:center;justify-content:center">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" stroke-width="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
            </div>
        ...
        <div id="chat-header" style="display:none;padding:12px 16px;border-bottom:1px solid var(--border);align-items:center;gap:10px;background:#F8FAFC;flex-wrap:wrap;flex-shrink:0">
            <button class="mc-back-btn" onclick="showConvList()" title="Back">&larr; Back</button>
            <div id="chat-avatar" style="width:36px;height:36px;background:linear-gradient(135deg,#0EA5E9,#3B82F6);border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px;flex-shrink:0"></div>
            <div style="flex:1;min-width:100px">
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
                    <div id="chat-name" style="font-weight:700;font-size:13px;color:#111827"></div>
                    <span id="chat-status-badge" class="mc-status-badge open" style="display:none">Open</span>
                </div>
                <div id="chat-code" style="font-size:11px;color:var(--text-muted)"></div>
            </div>
            <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
                <div class="mc-translate-group" style="height:28px">
        ...
        <!-- Messages -->
        <div id="chat-messages" class="mc-messages" style="display:none"></div>

        <!-- Input -->
        <div id="chat-input-area" style="display:none;padding:12px 16px;border-top:1px solid var(--border);background:#fff;flex-shrink:0">
            <div id="att-preview" style="display:none" class="mc-att-preview"></div>
            <div class="mc-input-row">
                <label class="mc-file-btn" title="Attach file (JPG, PNG, WEBP, PDF, CSV — max 10 MB)">
                ...
                <textarea id="chat-input" rows="1" placeholder="Type a reply..." onkeydown="chatKeyDown(event)" oninput="autoResize(this)"></textarea>
                <button onclick="sendMessage()" class="mc-send-btn">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                    <span>Send</span>
                </button>
            </div>
            <div style="font-size:11px;color:var(--text-muted);margin-top:5px">Press Ctrl+Enter to send · Attachments: JPG, PNG, WEBP, PDF, CSV (max 10 MB)</div>
        </div>
    </div>
</div>
...
function attExtLabel(m){
    var nm = String(m.attachment_name||''); var dot = nm.lastIndexOf('.');
...
function attachmentHtml(m){
    ...
    if ((m.attachment_type||'').indexOf('image/')===0) {
        return '<a href="'+dl+'" target="_blank"><img class="mc-att-img" src="'+dl+'" alt="'+name+'" loading="lazy" onload="if(!_userScrolledUp){scrollToBottom();}" onerror="this.style.display=\'none\'"></a>';
    }
    var ext = attExtLabel(m);
    var sizeTxt = m.attachment_size ? fmtBytes(m.attachment_size) : '';
    return '<a class="mc-att-file" href="'+dl+'" target="_blank" download>'+
        '<span class="mc-att-icon">'+escHtml(ext)+'</span>'+
        '<span><span class="mc-att-name">'+name+'</span>'+(sizeTxt?'<span style="display:block;font-size:11px;color:#94A3B8">'+escHtml(sizeTxt)+'</span>':'')+'</span></a>';
}
...
function renderReadReceipt(m) {
    if (parseInt(m.is_read) === 1 || m.status === 'read') {
        ...
        return '<span class="mc-read-receipt read" title="'+escHtml(tooltip)+'" style="margin-left:6px;display:inline-flex;align-items:center;gap:2px;color:#60A5FA;font-size:10px;font-weight:600">'+
        ...
        return '<span class="mc-read-receipt delivered" title="Delivered" style="margin-left:6px;display:inline-flex;align-items:center;color:rgba(255,255,255,0.7)">'+
        ...
        return '<span class="mc-read-receipt sent" title="Sent" style="margin-left:6px;display:inline-flex;align-items:center;color:rgba(255,255,255,0.7)">'+
...
function renderMsg(m) {
    ...
    div.className = 'mc-msg-row ' + (isMine ? 'mine' : 'theirs');
    div.dataset.id = m.id;
    ...
    var edited = m.edited_at ? '<span class="mc-edited">(edited)</span>' : '';
    var receipt = isMine ? renderReadReceipt(m) : '';
    var actions = '';
    if (isMine && (m.message||'').trim() !== '') {
        actions = '<div class="mc-actions"><button class="mc-action-btn" title="Edit" onclick="editMsg('+m.id+')">&#9998;</button></div>';
    }
    div.innerHTML =
        '<div class="mc-meta">'+escHtml(m.sender_name)+' · '+time+ edited + receipt +'</div>'+
        '<div class="mc-bubble" id="bubble-'+m.id+'">'+
            (m.message ? '<div class="mc-msg-text" data-orig="'+escHtml(m.message)+'">'+escHtml(m.message).replace(/\n/g,'<br>')+'</div>' : '')+
            attachmentHtml(m)+
        '</div>'+actions;
    return div;
}
...
function renderConvList(convs) {
    ...
    if (!convs.length) { list.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-muted);font-size:13px">No conversations yet</div>'; return; }
    var totalUnread = 0;
    list.innerHTML = '';
    convs.forEach(function(c){
        ...
        var el = document.createElement('div');
        el.className = 'mc-conv-row' + (isActive ? ' active' : '');
...
function showConvList(){
    document.getElementById('mc-conv-list-card').style.display='flex';
    document.getElementById('mc-chat-panel').style.display='none';
    document.getElementById('mc-chat-panel').classList.remove('mc-panel-active');
}

function selectConversation(affId, name, code, convId, status) {
    _selAffId = affId; _selConvId = convId||0; _selStatus = status||'open';
    ...
    if (window.innerWidth <= 780) {
        document.getElementById('mc-conv-list-card').style.display='none';
        document.getElementById('mc-chat-panel').style.display='flex';
        document.getElementById('mc-chat-panel').classList.add('mc-panel-active');
    }
            ...
            if (_renderedIds[m.id]) {
                ...
                if (existing) {
                    ...
                    if (oldRead !== String(m.is_read)) {
                        ...
                        var receiptEl = existing.querySelector('.mc-read-receipt');
...
function prepareUpload(input){
    ...
    var fd = new FormData();
    fd.append('_token', _csrfToken); fd.append('affiliate_id', _selAffId); fd.append('file', file);
...
function editMsg(id) {
    ...
    var textEl = bubbleEl.querySelector('.mc-msg-text');
...
function closeConv() {
    ...
    if (!confirm('Mark this conversation as solved?')) return;
...
function translateAllMessages() {
    ...
    var msgs = document.querySelectorAll('.mc-msg-text');
