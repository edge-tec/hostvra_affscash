...
.dash-header{padding:18px 18px;border-radius:12px;}
    ...
    .kpi-grid{grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)) !important;gap:12px;}
    ...
    try {
        $_dashFraud = Database::fetchAll(
            "SELECT cv.conversion_id, cv.payout, cv.converted_at, cv.country, cv.ip_address,
             ...
             WHERE cv.affiliate_id IN ($_in) AND cv.is_hidden = 0 AND (cv.hide_reason IS NULL OR cv.hide_reason NOT LIKE '%traffic_back%') AND NOT EXISTS (SELECT 1 FROM traffic_back_logs tbl WHERE tbl.click_id = cv.click_id) AND $_dashFraudRiskSql
             ORDER BY cv.converted_at DESC LIMIT 5",
            $_dashAffIds
        ) ?: [];
        $_dashFraudCount = (int)(Database::fetchOne(
            "SELECT COUNT(*) AS c FROM conversions cv WHERE cv.affiliate_id IN ($_in) AND cv.is_hidden = 0 AND (cv.hide_reason IS NULL OR cv.hide_reason NOT LIKE '%traffic_back%') AND NOT EXISTS (SELECT 1 FROM traffic_back_logs tbl WHERE tbl.click_id = cv.click_id) AND $_dashFraudRiskSql",
            $_dashAffIds
        )['c'] ?? 0);
    } catch (\Throwable $_e) {}
}
