...
.status-tab-row { display:flex; gap:0; border-bottom:2px solid #E2E8F0; margin-bottom:20px; flex-wrap:wrap; }
.status-tab { padding:9px 18px; font-size:13px; font-weight:600; color:#64748B; cursor:pointer;
              border-bottom:2px solid transparent; margin-bottom:-2px; text-decoration:none; transition:.15s; white-space:nowrap; }
...
<div class="status-tab-row">
    ...
    foreach ($tabs as $val => $label):
        $cntParams = $affIds;
        if (!empty($affIds)) {
            $cntIn  = implode(',', array_fill(0, count($affIds), '?'));
            $cntWhere = "affiliate_id IN ($cntIn)" . ($val !== 'all' ? " AND status='$val'" : '');
            $cnt = Database::fetchOne("SELECT COUNT(*) as c FROM affiliate_offers WHERE $cntWhere", $cntParams)['c'] ?? 0;
        } else { $cnt = 0; }
    ?>
...
<div class="card mb-3">
    <div class="card-body" style="padding:12px 16px">
        <form method="GET" action="/affiliate_manager/offer-approvals" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end">
            ...
            <div>
                ...
                <input type="text" name="aff" class="form-control" placeholder="Name, email or code…"
                       value="<?= Helpers::e($filterAff) ?>" style="min-width:200px">
            </div>
        ...
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
            ...
            <div>
                ...
                <div style="font-size:12px;color:#64748B">
                    <?= Helpers::e($req['affiliate_email']) ?> ·
                    <code style="background:#F1F5F9;padding:1px 6px;border-radius:3px;font-size:11px"><?= Helpers::e($req['affiliate_code']) ?></code>
                </div>
            </div>
        </div>
    ...
    <div class="oa-body">
        <div class="oa-grid">
            <div class="oa-field">
                ...
                <div class="val">
                    <span style="font-weight:700;color:#4F46E5"><?= Helpers::e($req['offer_name']) ?></span>
                </div>
                <div style="margin-top:3px">
                    ...
                    <?php if ($req['offer_category']): ?>
                    <span style="font-size:11px;color:#94A3B8;margin-left:4px"><?= Helpers::e($req['offer_category']) ?></span>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="oa-actions" style="margin-top:14px;padding-top:14px;border-top:1px solid #F1F5F9">
            <form method="POST" action="/affiliate_manager/offer-approvals" style="display:inline">
                <?= Helpers::csrf() ?>
            ...
            <a href="/affiliate_manager/affiliates?action=view&id=<?= $req['affiliate_id'] ?>" class="btn btn-secondary btn-sm">
                👤 View Profile
            </a>
        </div>
