...
try {
    $_dashFraud = Database::fetchAll(
        "SELECT cv.conversion_id, cv.payout, cv.converted_at, cv.country, cv.ip_address,
         ...
         WHERE cv.affiliate_id = ? AND cv.is_hidden = 0 AND (cv.hide_reason IS NULL OR cv.hide_reason NOT LIKE '%traffic_back%') AND (ck.source IS NULL OR ck.source != 'traffic_back') AND NOT EXISTS (SELECT 1 FROM traffic_back_logs tbl WHERE tbl.click_id = cv.click_id) AND $_dashFraudRiskSql
