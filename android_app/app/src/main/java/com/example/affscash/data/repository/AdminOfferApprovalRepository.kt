...
@Singleton
class AdminOfferApprovalRepository @Inject constructor(
    private val apiService: ApiService
) {
    suspend fun getOfferApprovals(status: String, offerId: Int?, aff: String?): Result<ManagerOfferApprovalListResponse> {
        return try {
            val response = apiService.getAdminOfferApprovals(status, offerId, aff)
            if (response.success) {
    ...
    suspend fun reviewOfferApproval(affiliateId: Int, offerId: Int, action: String): Result<DefaultResponse> {
        return try {
            ...
            val response = apiService.reviewAdminOfferApproval(request)
            if (response.success) {
                Result.success(response)
            } else {
