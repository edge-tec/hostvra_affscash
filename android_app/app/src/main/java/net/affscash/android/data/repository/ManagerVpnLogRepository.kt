...
@Singleton
class ManagerVpnLogRepository @Inject constructor(
    ...
    suspend fun getVpnLogs(
        ...
        try {
            val response = apiService.getManagerVpnLogs(ip, affiliate, type, dateFrom, dateTo)
    ...
    suspend fun getVpnLogStats(): Result<VpnLogStatsResponse> = withContext(Dispatchers.IO) {
        try {
            val response = apiService.getManagerVpnLogStats()
            if (response.isSuccessful) {
                response.body()?.let {
                    Result.success(it)
                } ?: Result.failure(Exception("Empty response body"))
    ...
    suspend fun clearOldLogs(): Result<BasicResponse> = withContext(Dispatchers.IO) {
        try {
            val response = apiService.clearManagerVpnLogs()
            if (response.isSuccessful) {
                response.body()?.let {
                    Result.success(it)
                } ?: Result.failure(Exception("Empty response body"))
