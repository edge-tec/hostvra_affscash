<?php
// Auto Rules Engine — alias of RiskEngineController (same data, different nav context)
Auth::check('admin');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_rule') {
        ...
        try {
            ...
            $message = 'Rule ' . ($newVal ? 'enabled' : 'disabled') . '.';
    ...
    } elseif ($action === 'reset_trigger_count') {
        $ruleId = (int)($_POST['rule_id'] ?? 0);
        try {
            Database::query("UPDATE fraud_rules SET trigger_count=0, last_triggered=NULL WHERE id=?", [$ruleId]);
            $message = 'Trigger count reset.';
        } catch (\Exception $e) { $error = 'Reset failed.'; }
    }
}
...
$rules = Database::fetchAll(
    "SELECT * FROM fraud_rules WHERE created_at BETWEEN ? AND ?
     ORDER BY is_active DESC, trigger_count DESC, id ASC",
...
$stats = [
    ...
    'inactive' => count(array_filter($rules, fn($r) => !$r['is_active'])),
    'triggers' => array_sum(array_column($rules, 'trigger_count')),
];

require BASE_PATH . '/views/admin/fraud_center/auto_rules.php';
