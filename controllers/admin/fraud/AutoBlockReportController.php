...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    if ($type === 'conversions') {
        fputcsv($f, ['ID','IP','Offer','Affiliate Code','Affiliate Name','Payout','Revenue','Status','Block Reason','Converted At']);
        $exp = Database::fetchAll("SELECT cv.id, cv.ip_address, o.name AS offer_name, a.affiliate_code, CONCAT(u.first_name, ' ', u.last_name) AS affiliate_name, cv.payout, cv.revenue, cv.status, cv.hide_reason, cv.converted_at FROM conversions cv LEFT JOIN offers o ON o.id=cv.offer_id LEFT JOIN affiliates a ON a.id=cv.affiliate_id LEFT JOIN users u ON u.id=a.user_id WHERE $convWhere ORDER BY cv.converted_at DESC LIMIT 5000", $convParams) ?: [];
        foreach ($exp as $r) fputcsv($f, [$r['id'],$r['ip_address'],$r['offer_name'],$r['affiliate_code'],$r['affiliate_name'],number_format($r['payout'],4),number_format($r['revenue'],4),$r['status'],$r['hide_reason'],$r['converted_at']]);
    } else {
        fputcsv($f, ['ID','IP','User Agent','Offer','Affiliate Code','Affiliate Name','Score','Clicked At']);
        $exp = Database::fetchAll("SELECT c.id, c.ip_address, c.user_agent, o.name AS offer_name, a.affiliate_code, CONCAT(u.first_name, ' ', u.last_name) AS affiliate_name, c.fraud_score, c.clicked_at FROM clicks c LEFT JOIN offers o ON o.id=c.offer_id LEFT JOIN affiliates a ON a.id=c.affiliate_id LEFT JOIN users u ON u.id=a.user_id WHERE $clickWhere ORDER BY c.clicked_at DESC LIMIT 5000", $clickParams) ?: [];
        foreach ($exp as $r) fputcsv($f, [$r['id'],$r['ip_address'],$r['user_agent'],$r['offer_name'],$r['affiliate_code'],$r['affiliate_name'],$r['fraud_score'],$r['clicked_at']]);
    }
    fclose($f); exit;
}
...
$recentBlockedClicks = Database::fetchAll(
    "SELECT c.id, c.ip_address, c.user_agent, c.clicked_at, c.offer_id,
            o.name AS offer_name, a.affiliate_code, CONCAT(u.first_name, ' ', u.last_name) AS affiliate_name, c.fraud_score
     FROM clicks c
...
$recentBlockedConversions = Database::fetchAll(
    "SELECT cv.id, cv.ip_address, cv.payout, cv.revenue, cv.status,
            cv.converted_at, o.name AS offer_name, a.affiliate_code, CONCAT(u.first_name, ' ', u.last_name) AS affiliate_name, cv.hide_reason, cv.fraud_score
     FROM conversions cv
